...
async def test_api_format():
    ...
    try:
        ...
        if post1_data.get('body'):
            body1 = json.loads(post1_data['body']) if isinstance(post1_data['body'], str) else post1_data['body']
            print(f"Post 1 content blocks: {len(body1.get('content', []))}")
        
        if post2_data.get('body'):
            body2 = json.loads(post2_data['body']) if isinstance(post2_data['body'], str) else post2_data['body']
