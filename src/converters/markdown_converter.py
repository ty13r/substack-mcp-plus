...
class MarkdownConverter:
    ...
    def _parse_code_block(self, lines: List[str], start: int) -> Tuple[Optional[Dict[str, Any]], int]:
        """Parse a code block"""
    ...
    def _parse_blockquote(self, lines: List[str], start: int) -> Tuple[Optional[Dict[str, Any]], int]:
        """Parse a blockquote"""
        quote_lines = []
        i = start
    ...
    def _parse_list(self, lines: List[str], start: int) -> Tuple[Optional[Dict[str, Any]], int]:
    ...
    def _parse_paragraph(self, lines: List[str], start: int) -> Tuple[Optional[Dict[str, Any]], int]:
        ...
        if paragraph_lines:
            ...
            img_match = re.match(r'^!\[([^\]]*)\]\(([^\s]+)(?:\s+"([^"]+)")?\)$', text.strip())
            if img_match:
        ...
        while remaining:
            ...
            if next_match:
                ...
                if next_match.start() > 0:
                    ...
                    elements.append(self._restore_escaped_chars({"type": "text", "content": plain_text}))
                
                # Add the formatted element
                if next_type == 'bold_italic':
                    elements.append(self._restore_escaped_chars(self.builder.text(next_match.group(1), ["strong", "em"])))
                elif next_type == 'bold':
                    elements.append(self._restore_escaped_chars(self.builder.text(next_match.group(1), ["strong"])))
                elif next_type == 'italic':
                    elements.append(self._restore_escaped_chars(self.builder.text(next_match.group(1), ["em"])))
                elif next_type == 'link':
                    elements.append(self._restore_escaped_chars(self.builder.link(next_match.group(1), next_match.group(2))))
                elif next_type == 'code':
                    elements.append(self._restore_escaped_chars(self.builder.text(next_match.group(1), ["code"])))
                
                remaining = remaining[next_match.end():]
            else:
                # No more formatting, add the rest as plain text
                elements.append(self._restore_escaped_chars({"type": "text", "content": remaining}))
                break
    ...
    def _restore_escaped_chars(self, element: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "content" in element and isinstance(element["content"], str):
            element["content"] = element["content"].replace('\x00ESCAPED_ASTERISK\x00', '*')
            element["content"] = element["content"].replace('\x00ESCAPED_BRACKET_OPEN\x00', '[')
            element["content"] = element["content"].replace('\x00ESCAPED_BRACKET_CLOSE\x00', ']')
        return element