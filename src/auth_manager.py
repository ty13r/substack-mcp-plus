...
class AuthManager:
    ...
    def _get_or_create_cipher(self) -> Fernet:
        ...
        if key_data:
            ...
            keyring.set_password(
                self.SERVICE_NAME, self.ENCRYPTION_KEY, base64.b64encode(key).decode()
            )
    ...
    def get_token(self) -> Optional[str]:
        ...
        encrypted_data = keyring.get_password(
            self.SERVICE_NAME, f"{self.TOKEN_KEY}:{self.publication_url}"
        )
        ...
        metadata_str = keyring.get_password(
            self.SERVICE_NAME, f"{self.METADATA_KEY}:{self.publication_url}"
        )
