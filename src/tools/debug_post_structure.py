    ...
    try:
        ...
        if isinstance(post, dict):
            ...
            for field in ["title", "draft_title", "subtitle", "draft_subtitle"]:
                if field in post:
                    debug_info["title_fields"][field] = post[field][:50] + "..." if len(str(post[field])) > 50 else post[field]
            ...
            for body_field in ["body", "draft_body"]:
                if body_field in post:
                    ...
                    debug_info["body_analysis"][body_field] = {
                        ...
                        "length": len(body) if isinstance(body, (str, list, dict)) else None
                    }
                    ...
                    if isinstance(body, dict) and "blocks" in body:
                        ...
                        debug_info["body_analysis"][body_field]["blocks_info"] = {
                            ...
                            "first_block": blocks[0] if isinstance(blocks, list) and blocks else None
                        }
                    
                    # Sample content
                    if isinstance(body, str):
                        debug_info["sample_content"][body_field] = body[:200] + "..." if len(body) > 200 else body
                    elif isinstance(body, dict):
                        debug_info["sample_content"][body_field] = json.dumps(body, indent=2)[:500] + "..."
